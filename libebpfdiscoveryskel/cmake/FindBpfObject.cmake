...
file, generates an interface library target, ``<name>``, that may be
...
::
	...
	target_link_libraries(myapp myobject)
...
execute_process(
	COMMAND
		bash -c "${BPFOBJECT_CLANG_EXE} -v -E - < /dev/null 2>&1 |
...
# Public macro
macro(bpf_object library input)
	...
	set(OUTPUT_TARGET ${name}skel)

	# Build BPF object file
	add_custom_command(
		OUTPUT ${BPF_O_FILE}
		COMMAND
			${BPFOBJECT_CLANG_EXE} -g -O2 -target bpf -D__TARGET_ARCH_${ARCH} ${BPF_C_FLAGS} ${CLANG_SYSTEM_INCLUDES}
			-I${GENERATED_VMLINUX_DIR} -isystem ${LIBBPF_INCLUDE_DIRS} -c ${BPF_C_FILE} -o ${BPF_O_FILE}
	...
	add_custom_target(${OUTPUT_TARGET} DEPENDS ${BPF_SKEL_FILE})

	add_library(${library} INTERFACE)
	add_dependencies(${library} ${OUTPUT_TARGET})
	target_include_directories(${library} INTERFACE ${CMAKE_CURRENT_BINARY_DIR})
	target_include_directories(${library} SYSTEM INTERFACE ${LIBBPF_INCLUDE_DIRS})
	target_link_libraries(${library} INTERFACE ${LIBBPF_LIBRARIES} -lelf -lz)
endmacro()
