...
__attribute__((always_inline)) inline static void handleAcceptIPv4Session(
	...
	DEBUG_PRINTLN("saved ipv4 pid: `%d`, fd: `%d`, addr: `%d`", trackedSessionKey.pid, trackedSessionKey.fd, acceptArgsPtr->addr);
}
...
__attribute__((always_inline)) inline static void handleAcceptIPv6Session(
	...
	DEBUG_PRINTLN("saved ipv6 pid: `%d`, fd: `%d`, addr: `%d`", trackedSessionKey.pid, trackedSessionKey.fd, acceptArgsPtr->addr);
}

__attribute__((always_inline)) inline static void handleAccept(
		struct pt_regs* ctx, __u64 pidTgid, struct AcceptArgs* acceptArgsPtr, int fd) {
	...
	switch (saFamily) {
	case AF_INET:
		handleAcceptIPv4Session(ctx, trackedSessionKey, pidTgid, acceptArgsPtr, addrlen);
		break;
	case AF_INET6:
		handleAcceptIPv6Session(ctx, trackedSessionKey, pidTgid, acceptArgsPtr, addrlen);
		break;
	}
}
...
__attribute__((always_inline)) inline static void fillTrackedSessionAndPushEvent(
	...
	struct DiscoveryEvent event = {
			.key = *key,
			.sourceIP = sessionPtr->sourceIP,
			.flags = sessionPtr->flags | DISCOVERY_FLAG_EVENT_NEW_DATA,
	};
...
__attribute__((always_inline)) inline static void handleNoMoreData(
	...
	struct DiscoveryEvent event = {
			.key = {.pid = key.pid, .fd = fd, .sessionID = sessionPtr->id, .bufferSeq = sessionPtr->bufferCount},
